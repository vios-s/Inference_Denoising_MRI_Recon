...
class AdaptiveSensitivityModel(nn.Module):
    ...
    def forward(self, masked_kspace: torch.Tensor, mask: torch.Tensor) -> torch.Tensor:
        ...
        x = fastmri_transforms.batched_mask_center(masked_kspace, pad, pad + num_low_freqs)
...
class AdaptiveVarNet(nn.Module):
    ...
    def forward(
        ...
        output = rss(complex_abs(ifft2c(kspace_pred)), dim=1)
    ...
    def extract_low_freq_mask(self, mask: torch.Tensor, masked_kspace: torch.Tensor):
        ...
        mask = fastmri_transforms.batched_mask_center(mask, pad, pad + num_low_freqs)
        masked_kspace = fastmri_transforms.batched_mask_center(
